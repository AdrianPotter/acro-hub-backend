...
resource "aws_api_gateway_resource" "videos_view" {
  rest_api_id = aws_api_gateway_rest_api.acro_hub.id
  parent_id   = aws_api_gateway_resource.videos_move_id.id
  path_part   = "view"
}

# POST /videos/{moveId}/view

resource "aws_api_gateway_method" "videos_view_post" {
  rest_api_id   = aws_api_gateway_rest_api.acro_hub.id
...
resource "aws_api_gateway_integration" "videos_view_post" {
  rest_api_id             = aws_api_gateway_rest_api.acro_hub.id
  resource_id             = aws_api_gateway_resource.videos_view.id
  http_method             = aws_api_gateway_method.videos_view_post.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/${aws_lambda_function.videos.arn}/invocations"
}
...
resource "aws_api_gateway_integration" "videos_view_options" {
  rest_api_id = aws_api_gateway_rest_api.acro_hub.id
  resource_id = aws_api_gateway_resource.videos_view.id
  http_method = aws_api_gateway_method.videos_view_options.http_method
  type        = "MOCK"
  request_templates = {
    "application/json" = "{\"statusCode\": 200}"
  }
...
resource "aws_api_gateway_method_response" "videos_view_options_200" {
  rest_api_id = aws_api_gateway_rest_api.acro_hub.id
  resource_id = aws_api_gateway_resource.videos_view.id
  http_method = aws_api_gateway_method.videos_view_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "videos_view_options" {
  rest_api_id = aws_api_gateway_rest_api.acro_hub.id
  resource_id = aws_api_gateway_resource.videos_view.id
  http_method = aws_api_gateway_method.videos_view_options.http_method
  status_code = aws_api_gateway_method_response.videos_view_options_200.status_code

  depends_on = [
    aws_api_gateway_integration.videos_view_options
  ]
